...
export function HomeSection({ onRelogin, onLogout }: HomeSectionProps) {
	...
	const [wsImportMsg, setWsImportMsg] = useState<{
		type: "success" | "error";
		text: string;
	} | null>(null);
	...
	const [welcomeDismissed, setWelcomeDismissed] = useState(
		() => localStorage.getItem("codeck-welcome-dismissed") === "1",
	);
			...
			<div class="home-content">
				{dockerExperimental.value && (
					<div class="experimental-warning" role="alert">
						...
						<div class="experimental-warning-text">
							...
							<p>
								Docker socket is mounted. The container has full access to the
								host Docker daemon.
							</p>
						</div>
					</div>
				...
				{showWelcome && (
					<div class="welcome-card">
						...
						<p class="welcome-subtitle">
							Your AI coding agent with persistent memory
						</p>
						<ul class="welcome-features">
							<li>
								...
								<span>
									<strong>Agent Memory</strong> — Claude remembers your
									projects, preferences, and decisions across sessions
								</span>
							</li>
							<li>
								<span class="welcome-feature-icon" aria-hidden="true">
								...
								<span>
									<strong>Always On</strong> — Your machine runs 24/7,
							...
							<li>
								...
								<span>
									<strong>Skills &amp; Agents</strong> — Pre-loaded knowledge
									packs and sub-agents that make Claude smarter
								</span>
							</li>
						</ul>
						<button
							class="btn btn-primary welcome-cta"
							onClick={() => {
								setActiveSection("claude");
								history.pushState(null, "", "/claude");
							}}
						>
					...
					<div class="dash-card">
						...
						<div class="home-account-limits">
							...
							{usage?.available ? (
										...
										<DashBar
											...
											detail={
												usage.fiveHour.resetsAt
													? `resets ${formatTimeUntil(usage.fiveHour.resetsAt)}`
													: ""
											}
										/>
									)}
										...
										<DashBar
											...
											detail={
												usage.sevenDay.resetsAt
													? `resets ${formatTimeUntil(usage.sevenDay.resetsAt)}`
													: ""
											}
										/>
									)}
								...
								<div
									class="home-account-limits-loading"
								...
								<div class="home-account-limits-loading">
									<span class="text-muted">
										Not available — usage data requires an active Claude session
									</span>
								</div>
							)}
						</div>
						...
						{showRelogin && (
							<div class="home-account-relogin">
								...
								<button class="btn btn-xs btn-secondary" onClick={onRelogin}>
									Re-login
								</button>
							</div>
						)}
						...
						{dashLoading && !dashboard ? (
							...
							<>
								<div class="dash-bars">
									<DashBar
										label="CPU"
										percent={dashboard.resources.cpu.usagePercent}
										detail={`${dashboard.resources.cpu.cores} cores`}
									/>
									<DashBar
										label="Memory"
										percent={dashboard.resources.memory.percent}
										detail={`${formatBytes(dashboard.resources.memory.used)} / ${formatBytes(dashboard.resources.memory.limit)}`}
									/>
									<DashBar
										label="Disk"
										percent={dashboard.resources.disk.percent}
										detail={`${formatBytes(dashboard.resources.disk.used)} / ${formatBytes(dashboard.resources.disk.total)}`}
									/>
								</div>
								<div class="dash-meta">
									Sessions: {dashboard.resources.sessions}/5 &nbsp;|&nbsp;
									Ports: {dashboard.resources.ports} &nbsp;|&nbsp; Uptime:{" "}
									{formatUptime(dashboard.resources.uptime)}
								</div>
								{ports.length > 0 && (
									<div class="dash-ports">
										{ports.map((p) => {
											...
											return (
												<a
													key={port}
													class={`dash-port-link${exposed ? "" : " unexposed"}`}
													href={href}
													target="_blank"
							...
							<div class="dash-meta" style="border-top: none">
				...
				<div class="home-filesystem">
					...
					<div class="home-export">
						...
						<button
							class="btn btn-xs btn-secondary"
							onClick={() => wsImportRef.current?.click()}
						...
						/>
						{wsImportMsg && (
							<span class={`mem-quick-msg mem-quick-msg-${wsImportMsg.type}`}>
								{wsImportMsg.text}
							</span>
						)}
					</div>
				...
				<div class="modal-overlay" onClick={() => setShowChangePassword(false)}>
					...
					>
						<div class="modal-title">
							<IconKey size={16} /> Change Password
						</div>
						<div class="form-group">
							...
							<input
								...
								value={pwCurrent}
								onInput={(e) =>
									setPwCurrent((e.target as HTMLInputElement).value)
								}
