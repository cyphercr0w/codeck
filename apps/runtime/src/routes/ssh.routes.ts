...
router.get("/status", (_req, res) => {
	const hasKey = hasSSHKey();
	res.json({
		hasKey,
		publicKey: hasKey ? getSSHPublicKey() : null,
		authenticated: hasKey ? testSSHConnection() : false,
	});
});
...
router.get("/public-key", (_req, res) => {
	...
	if (publicKey) res.json({ success: true, publicKey });
		...
		res.status(500).json({ success: false, error: "Could not get public key" });
});
