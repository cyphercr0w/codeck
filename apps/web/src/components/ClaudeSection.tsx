...
import {
	...
	scrollToBottom,
	getTerminal,
	markSessionAttaching,
	clearSessionAttaching,
	onTerminalWrite,
...
export function ClaudeSection({
		...
		setTerminalHandlers(
			...
			(sessionId) => {
				...
				if (
					...
					new Notification("Codeck", {
						body: `Terminal "${cwdShort}" finished`,
					});
				}
	...
	useEffect(() => {
		...
		for (const s of sessionList) {
			...
			el.className =
				s.id === activeId ? "terminal-instance active" : "terminal-instance";
			if (s.id === activeId) {
				container
			...
			<div class="tsb-left">
				...
				{usage?.available && usage.fiveHour && (
					<div class="tsb-limit-group">
						...
						{usage.fiveHour.resetsAt && (
							<span class="tsb-limit-reset">
								resets {formatReset(usage.fiveHour.resetsAt)}
							</span>
						)}
					</div>
				)}
				...
				{usage?.available && usage.sevenDay && (
					<div class="tsb-limit-group">
						...
						{usage.sevenDay.resetsAt && (
							<span class="tsb-limit-reset">
								resets {formatReset(usage.sevenDay.resetsAt)}
							</span>
						)}
					</div>
				)}
				...
				{ctx && ctx.contextPercent > 0 && (
					<>
						<span class="tsb-sep">|</span>
						<div class="tsb-limit">
							<span class="tsb-limit-label tsb-ctx-label">CTX</span>
...
export function mountTerminalForSession(
	sessionId: string,
	cwd: string,
	name?: string,
) {
		...
		if (data.sessions && data.sessions.length > 0) {
			for (const s of data.sessions) {
				addSession({
					...
					type: s.type || "agent",
					cwd: s.cwd,
					name: s.name || s.cwd.split("/").pop() || s.cwd,
