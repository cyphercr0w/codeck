...
export async function initializeIndexer(): Promise<boolean> {
	try {
		...
		try {
			...
			const sqliteVec = await (Function(
				'return import("sqlite-vec")',
			)() as Promise<any>);
			sqliteVec.load(db);
      ...
      `);
			...
			console.log(
				"[Indexer] sqlite-vec not available (vector search disabled):",
				(e as Error).message,
			);
		}
		...
		// Schedule daily SQLite backup (every 24 hours)
		setInterval(
			() => {
				backupDatabase();
			},
		...
		console.log(
			"[Indexer] SQLite not available (expected on Windows dev):",
			(e as Error).message,
		);
		available = false;
		return false;
...
function chunkMarkdown(
	...
	return chunks.length > 0
		? chunks
		: [{ content: content.trim(), metadata: JSON.stringify(meta) }];
}
...
function getFileType(filepath: string): string {
	...
	if (rel.startsWith("paths/") || rel.startsWith("paths\\")) {
		...
		if (rel.includes("/daily/") || rel.includes("\\daily\\"))
			return "path-daily";
...
function getRelPath(filepath: string): string {
	if (filepath.startsWith(PATHS.SESSIONS_DIR)) {
		return (
			"sessions/" + relative(PATHS.SESSIONS_DIR, filepath).replace(/\\/g, "/")
		);
	}
...
function indexFile(filepath: string): void {
	...
	const existing = db
		...
		.get(relPath) as { id: number; hash: string } | undefined;
	...
	const chunks =
		ext === ".jsonl" ? chunkJsonl(content, meta) : chunkMarkdown(content, meta);
...
export function indexAll(): { success: boolean; reason?: string } {
	...
	if (reindexInProgress)
		return { success: false, reason: "Reindex already in progress" };
...
function _indexAllInner(): { success: boolean } {
	...
	const allPaths = db.prepare("SELECT id, path FROM files").all() as {
		id: number;
		path: string;
	}[];
...
function startWatchers(): void {
						...
						try {
							...
							if (code === "ENOENT") {
								...
								if (db) {
									...
									if (row) {
										db.prepare("DELETE FROM chunks WHERE file_id = ?").run(
											row.id,
										);
										db.prepare("DELETE FROM files WHERE id = ?").run(row.id);
									}
	...
	if (watchers.length > 0)
		console.log(`[Indexer] File watchers started (${watchers.length} dirs)`);
}
...
export async function processEmbeddingQueue(): Promise<number> {
	...
	for (const item of batch) {
		try {
			...
			console.log(
				"[Indexer] Embedding error for chunk",
				item.chunkId,
				":",
				(e as Error).message,
			);
	...
	if (indexed > 0) {
		console.log(
			`[Indexer] Embedded ${indexed}/${batch.length} chunks (${embeddingQueue.length} remaining)`,
		);
	}
...
const BACKUPS_DIR = join(PATHS.CODECK_DIR, "backups");
const MAX_BACKUPS = 7;

/**
 * Create a daily backup of the SQLite database.
 * Flushes the WAL, copies the DB file, and keeps only the last 7 backups.
 * Can be called manually via API or runs automatically every 24 hours.
 */
export function backupDatabase(): {
	...
	try {
		...
		// Copy database file to backup with date stamp
		const date = new Date().toISOString().slice(0, 10);
		const backupPath = join(BACKUPS_DIR, `memory-${date}.sqlite`);
		...
		console.log(
			`[Indexer] Database backed up to ${backupPath} (${backups.length} total, keeping ${MAX_BACKUPS})`,
		);
		return { success: true, path: backupPath };
	} catch (e) {
		const msg = (e as Error).message;
		console.log("[Indexer] Database backup failed:", msg);
...
export function getIndexStats(): Record<string, unknown> {
	...
	const typeCounts = db
		.prepare("SELECT type, COUNT(*) as c FROM files GROUP BY type")
	...
	if (vecAvailable) {
		try {
			vecCount = (
				db.prepare("SELECT COUNT(*) as c FROM chunks_vec").get() as {
					c: number;
				}
			).c;
		} catch {
