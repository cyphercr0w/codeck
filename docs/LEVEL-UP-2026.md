...
- ✅ **Capability preflight / graceful degradation** (`capability-doctor.mjs`) — probes external capabilities (`gh` auth, git remote, Docker daemon, node/pnpm/python, headless browser, network egress, disk) with **real execution, not `which`**, caches to `.codeck/state/capabilities.json` (SessionStart, async), and injects an `<environment-capabilities>` preflight (only NOT-ready capabilities + their fallbacks) on the first message. The `autonomous-harness` runs it as a preflight before a long run; `rules/base/workflow.md` codifies the `which`≠ready + ordered-fallback convention. Complements `ecc/mcp-health-check.js` (MCP-server health) for the non-MCP tool surface. *Pattern adapted from Panniantong/agent-reach (MIT) — its `doctor` + backend-fallback idea, applied to harness robustness, not its scrapers.*
...
- ✅ **ECC v2.0 resync ports + MCP hygiene** (2026-07): audited upstream `affaan-m/ecc` v2.0.0 HEAD and ported only the harness-robustness wins (the rest of v2.0 is business/media/multi-CLI, skipped). Shipped: **MCP default-connector reduction** (mirrors ecc's post-audit posture) — cut defaults to `context7`/`eslint`/`playwright`; `memory` (redundant with native FTS5 memory, zero consumers), `magic` (niche UI), `sequential-thinking` moved to opt-in `_disabledMcpServers` → ~12 fewer tool-schemas/session (token tax + injection surface), Playwright kept as the browser-verify differentiator. **`agent-introspection-debugging` skill** — capture→diagnose(logic/state/env/policy)→contained-fix→re-plan/ESCALATE on the 2nd repeat, killing retry-thrash (top autonomy failure). **`silent-failure-hunter` agent** — read-only reviewer for swallowed exceptions / masked failures the general reviewer misses. **Fail-closed review contract** (from ecc's `orch-review`): dedup findings on normalized evidence, adversarially verify each CRITICAL/HIGH, never signal clean on an incomplete review. **Autonomous context lifecycle** rule — compact at phase boundaries and resume from `.codeck/harness` state; a full context is never a reason to stop. Deferred (noted, higher effort): worktree-per-worker orchestration, `instinct`/`evolve` crystallization, `plan-canvas` browser plan-review, AgentShield (external — scans the harness config itself).
- ✅ **Product-Owner-driven autonomy** (the overnight-build differentiator): a `product-owner` overseer agent (read-only, `opus`) renders the decisions a human sponsor would — **plan verdict** (after a plan-review loop: `spec-reviewer`+`architect`+`grader` to zero findings), **each review/audit round** (loop to zero findings, clean markers), and **DONE** (evidence-gated). No user sign-off between the initial questionnaire and delivery; only the PO's `ESCALATE` returns control to a human. The `workflow-checkpoint.mjs` Stop hook becomes the **self-reprompt keep-alive**: while a harness task is active and not DONE/ESCALATED, a premature stop is blocked and auto-reprompted with the pending directive — bounded by `budget-guard` (hard cap) + a reprompt backstop + `no-progress-guard`. An initial `AskUserQuestion` chooses autonomous (default) vs supervised. Non-harness quick tasks keep the legacy review/audit gate (no over-blocking). Files: `ecc/agents/product-owner.md`, `workflow-checkpoint.mjs`, `review-marker-hook.mjs` (clean/findings), `rules/base/workflow.md` + `playbooks.md`, `autonomous-protocol-hook.mjs`, `autonomous-harness` skill.
