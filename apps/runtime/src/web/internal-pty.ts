	...
	wss.on("connection", (ws, req) => {
		...
		ws.on("message", (raw) => {
			try {
				...
				console.warn(
					"[Internal PTY] Failed to parse message:",
					(e as Error).message,
				);
			}
		});
		...
		ws.on("close", () => {
			console.log(
				`[Internal PTY] Client disconnected from session ${sessionId}`,
			);
...
export function handlePtyUpgrade(
	...
	// Validate session exists before upgrading — the session UUID acts as an
	// implicit auth token (only legitimate callers know it).
	const pathname = (req.url || "").split("?")[0];
	const parts = pathname.split("/");
	const sessionId = parts[3];
	if (!sessionId || !UUID_RE.test(sessionId) || !getSession(sessionId)) {
