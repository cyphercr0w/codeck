...
router.post('/', async (req, res) => {
  try {
    ...
    if (env && typeof env === 'object' && !Array.isArray(env)) serverConfig.env = env;
    if (description && typeof description === 'string') serverConfig.description = description;
...
router.post('/:name/toggle', async (req, res) => {
  try {
    ...
    if (config.mcpServers[name]) {
      ...
      delete config.mcpServers[name];
      config._disabledMcpServers[name] = serverConfig;
      await writeClaudeJson(config);
...
router.get('/search', async (req, res) => {
  try {
    ...
    if (!response.ok) {
      res.status(502).json({ success: false, error: `Registry returned ${response.status}` });
      return;
    }
