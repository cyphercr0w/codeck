...
// ── XML tag mapping (static, never mutated) ─────────────────────
const STATIC_TAG_MAP: Readonly<Record<string, string>> = {
 ...
 */
export function buildSessionContext(cwd: string): {
	context: string;
	...
	const addPart = (
		...
		const budgetMap = {
			hot: HOT_BUDGET - hotUsed,
			warm: WARM_BUDGET - warmUsed,
			cold: COLD_BUDGET - coldUsed,
		};
	...
	try {
		...
		/* path resolution can fail on first use */
	}
	...
	if (pathId) {
		...
		if (pathDaily.content) {
			addPart(
				"Project Today",
				trimDaily(pathDaily.content, WARM_BUDGET / 2),
				"warm",
				"path",
			);
		}
	...
	// Use fixed tag names (not date-keyed) to avoid mutating the static tag map.
	const todayEntry = getDailyEntry(today);
	if (todayEntry.content) {
		addPart(
			"Today Activity",
			trimDaily(todayEntry.content, WARM_BUDGET / 2),
			"warm",
			"daily",
		);
	} else {
		...
		if (yesterdayEntry.content) {
			addPart(
				"Yesterday Activity",
				trimDaily(yesterdayEntry.content, WARM_BUDGET / 2),
				"warm",
				"daily",
			);
		}
	...
	console.log(
		`[MemoryContext] Budget: HOT ${hotUsed}/${HOT_BUDGET}, WARM ${warmUsed}/${WARM_BUDGET}, COLD ${coldUsed}/${COLD_BUDGET}, total ${totalUsed}/${TOTAL_BUDGET}`,
	);
...
function removeContextSection(): void {
	...
	if (startIdx !== -1 && endIdx !== -1) {
		...
		content =
			content.slice(0, removeFrom) + content.slice(endIdx + MARKER_END.length);
