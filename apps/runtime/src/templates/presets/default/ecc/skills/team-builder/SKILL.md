...
- **Parallel Agent calls.** This skill uses parallel Agent tool calls for independent work. When agents must debate or respond to each other, give each a unique `name` (making it a persistent teammate) and coordinate with `SendMessage` — every session is already an implicit team since Claude Code 2.1.178 (the old `TeamCreate`/`TeamDelete` tools were removed).
