...
export function initFlowTemplates(): void {
	...
	if (!existsSync(codeReviewPath)) {
		...
		const template: FlowDefinition = {
			...
			name: "Code Review",
			description:
				"Reviews code first, then implements fixes if needed. Loops until reviewer approves.",
			version: "2.0.0",
			isTemplate: true,
			createdAt: now,
			updatedAt: now,
			entryAgentId: "reviewer",
			agents: {
				reviewer: {
					id: "reviewer",
					name: "Code Reviewer",
					...
					systemPrompt:
						"You are an expert code reviewer. Read the code specified by the user, analyze it thoroughly for bugs, type safety, error handling, security, performance, and best practices. Provide a detailed review with file:line references.\n\nAt the end of your review, write one of:\n- DECISION: APPROVE — if code is clean, no issues found\n- DECISION: REQUEST_CHANGES — if there are issues that need fixing\n\nList each issue with severity (CRITICAL/HIGH/MEDIUM) and a clear description of what to fix.",
				...
				implementer: {
					...
					systemPrompt:
						"You are a senior developer. You receive a code review with issues to fix. Read the review carefully, then fix every issue mentioned. After fixing, run the build and tests to verify nothing is broken.\n\nFix all CRITICAL and HIGH issues. Fix MEDIUM issues when straightforward. Make minimal, focused changes — don't refactor unrelated code.",
