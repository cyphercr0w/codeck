...
const CACHE_TTL = 300_000; // 5 minutes — avoid hitting Anthropic rate limits
const RATE_LIMIT_BACKOFF = 600_000; // 10 min backoff after 429
...
function getOAuthToken(): string | null {
	...
	if (
		...
		isRealToken(process.env.CLAUDE_CODE_OAUTH_TOKEN)
	) {
...
export async function getClaudeUsage(): Promise<ClaudeUsage> {
	...
	try {
		...
		if (!res.ok) {
			...
			// On rate limit (429), extend cache TTL to back off and return stale data
			if (res.status === 429) {
				if (usageCache) {
					usageCache.fetchedAt = Date.now() - CACHE_TTL + RATE_LIMIT_BACKOFF;
					return usageCache.data;
				}
				return { available: false, fiveHour: null, sevenDay: null };
			}
