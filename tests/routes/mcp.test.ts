...
describe("MCP API", () => {
	...
	describe("GET /api/mcp", () => {
		...
		it("should mark active servers as enabled", async () => {
			...
			const context7 = res.body.servers.find(
				(s: { name: string }) => s.name === "context7",
			);
		...
		it("should mark disabled servers as not enabled", async () => {
			...
			const playwright = res.body.servers.find(
				(s: { name: string }) => s.name === "playwright",
			);
	...
	describe("POST /api/mcp", () => {
		it("should add a new MCP server with name and command", async () => {
			...
			expect(writtenData.mcpServers["new-server"].args).toEqual([
				"-y",
				"new-mcp-pkg",
			]);
		});
		...
		it("should update an existing server", async () => {
			...
			const writtenData = JSON.parse(mockWriteFile.mock.calls[0][1]);
			expect(writtenData.mcpServers["context7"].args).toEqual([
				"-y",
	...
	describe("DELETE /api/mcp/:name", () => {
		it("should remove an active server", async () => {
			const res = await request(app).delete("/api/mcp/eslint").expect(200);
		...
		it("should remove a disabled server", async () => {
			const res = await request(app).delete("/api/mcp/playwright").expect(200);
		...
		it("should return 404 for nonexistent server", async () => {
			const res = await request(app).delete("/api/mcp/nonexistent").expect(404);
	...
	describe("POST /api/mcp/:name/toggle", () => {
		it("should disable an active server", async () => {
			...
			const writtenData = JSON.parse(mockWriteFile.mock.calls[0][1]);
			expect(writtenData.mcpServers["context7"]).toBeDefined();
			expect(writtenData.mcpServers["context7"].disabled).toBe(true);
		});
		...
		it("should clean up empty _disabledMcpServers", async () => {
			...
			const res = await request(app).post("/api/mcp/lonely/toggle").expect(200);
	...
	describe("GET /api/mcp/search", () => {
		it("should return empty servers for empty query", async () => {
			const res = await request(app).get("/api/mcp/search").expect(200);
		...
		it("should return empty servers for whitespace-only query", async () => {
			const res = await request(app).get("/api/mcp/search?q=   ").expect(200);
		...
		it("should fetch and transform results from registry", async () => {
			...
			const res = await request(app).get("/api/mcp/search?q=test").expect(200);
		...
		it("should derive title from name when title is not provided", async () => {
			...
			const res = await request(app).get("/api/mcp/search?q=gmail").expect(200);
