...
function readCgroupFile(path: string): string | null {
	try {
		...
		console.debug(
			`[Resources] Failed to read ${path}: ${(e as Error).message}`,
		);
		return null;
	}
}
...
export function getContainerResources(): ContainerResources {
	...
	return {
		...
		memory: {
			...
			limit: mem.limit,
			percent:
				mem.limit > 0 ? Math.round((mem.used / mem.limit) * 1000) / 10 : 0,
		},
		disk: {
			used: disk.used,
			total: disk.total,
			percent:
				disk.total > 0 ? Math.round((disk.used / disk.total) * 1000) / 10 : 0,
		},
...
/**
 * Get current PID usage from cgroups.
 */
export function getPidUsage(): { current: number; limit: number } {
	const current = readCgroupFile("/sys/fs/cgroup/pids.current");
	const max = readCgroupFile("/sys/fs/cgroup/pids.max");

	return {
		current: current ? parseInt(current) || 0 : 0,
		limit: max && max !== "max" ? parseInt(max) || 0 : 0,
	};
}

/**
 * Get full process tree for crash diagnostics.
 */
export async function getProcessTree(): Promise<string> {
	try {
		const { stdout } = await execFileAsync("ps", ["auxf", "--no-headers"], {
			encoding: "utf-8",
			timeout: 5000,
		});
		return stdout;
