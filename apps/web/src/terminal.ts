...
export function createTerminal(
	...
	if (textarea) {
		...
		textarea.addEventListener(
			...
			(e: ClipboardEvent) => {
				...
				// Check for any file (image, video, PDF, doc, etc.)
		...
		const blurHandler = () => {
			...
			console.warn(
				`[xterm] ${sessionId.slice(0, 6)} textarea BLUR — input will freeze until re-focused. activeElement=${activeInfo}`,
			);
		};
	...
	if (xtermViewport) {
		xtermViewport.addEventListener(
			...
			() => {
				...
				const atBottom =
					xtermViewport.scrollTop + xtermViewport.clientHeight >=
					xtermViewport.scrollHeight - 10;
	...
	const resizeObserver = new ResizeObserver((entries) => {
		...
		if (
			Math.abs(w - lastContainerWidth) < 3 &&
			Math.abs(h - lastContainerHeight) < 3
		)
			return;
		...
		resizeTimer = setTimeout(
			() => {
				...
				const wasTerminalFocused =
					!isMobile.value && !!textarea && document.activeElement === textarea;
				fitAddon.fit();
				wsSend({
					type: "console:resize",
					sessionId,
...
export function fitTerminal(sessionId: string): void {
	...
	if (
		instance.container.offsetWidth === 0 ||
		instance.container.offsetHeight === 0
	)
		return;
	...
	const wasTerminalFocused =
		!isMobile.value &&
		!!instance.textarea &&
		document.activeElement === instance.textarea;
...
export function repaintTerminal(sessionId: string): void {
	...
	const viewport = instance.container.querySelector(
		".xterm-viewport",
	) as HTMLElement | null;
	if (viewport) {
		suppressScrollEvents(sessionId);
