...
import {
	...
	getSessionSummary,
} from "../services/session-writer.js";
import {
	search,
	isSearchAvailable,
	hybridSearch,
	...
	(req, res) =>
		fn(req, res).catch((err) => {
			...
			if (!res.headersSent)
				res
					.status(500)
					.json({ error: "Memory operation failed. Please try again." });
		});
...
function validPathId(
	...
	if (!clean) {
		res.status(400).json({ error: "Invalid pathId: must be 12-char hex" });
		return null;
	}
	return clean;
}
...
router.post("/decisions/create", (req, res) => {
	...
	const consequences = requireString(
...
router.post(
	...
	asyncHandler(async (req, res) => {
		...
		const targetScope = optionalString(
		...
		if (decision === null) return;
		const consequences = optionalString(
			req.body.consequences,
			"consequences",
			res,
			MAX_CONTENT_LENGTH,
		...
		const resolvedScope =
			targetScope === "path" && pathId ? pathId : targetScope || "global";
...
router.get("/search", (req, res) => {
	...
	if (q && q.length > 1000) {
		res
			.status(400)
			.json({ error: "Query exceeds maximum length (1000 characters)" });
		return;
	}
