	...
	asyncHandler(async (req, res) => {
		try {
			...
			if (typeof matcher !== "string") {
				res
					.status(400)
					.json({ success: false, error: "matcher must be a string" });
				return;
			}
			...
			const existing = settings.hooks[event].find(
				(m: HookMatcher) => m.matcher === matcher,
			);
			if (existing) {
		...
		try {
			...
			if (!eventHooks || index >= eventHooks.length) {
				res
					...
					.json({
						success: false,
						error: `Hook at index ${index} not found for event '${event}'`,
					});
				return;
			}
			...
			if (matcher !== undefined) {
				if (typeof matcher !== "string" || !matcher) {
					res
						...
						.json({
							success: false,
							error: "matcher must be a non-empty string",
						});
					return;
				}
