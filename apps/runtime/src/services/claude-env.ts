...
export function resolveAgentBinary(): string {
	...
	for (const locator of ["which", "where"]) {
		try {
			const result = execFileSync(locator, [ACTIVE_AGENT.command], {
				encoding: "utf8",
				timeout: 5000,
			})
				.trim()
...
export function getValidAgentBinary(): string {
	...
	console.log(
		`[claude-env] Agent binary missing at ${agentBinaryPath}, re-resolving...`,
	);
...
export function getOAuthEnv(): Record<string, string> {
	...
	// In-memory token expired — try to reload from credentials file.
	// The Claude CLI may have refreshed the token via refreshToken and written
	// a new accessToken to .credentials.json. Read it and update in-memory cache.
	const expiresAt = getInMemoryTokenExpiresAt();
	if (expiresAt > 0 && now >= expiresAt) {
		try {
			const creds = readCredentials();
			const freshToken = creds?.claudeAiOauth?.accessToken;
			const freshExpiry = creds?.claudeAiOauth?.expiresAt;
			if (
				freshToken &&
				isRealToken(freshToken) &&
				freshExpiry &&
				freshExpiry > now
			) {
				// Credentials file has a fresh token — update in-memory cache
				setInMemoryToken(freshToken, freshExpiry);
				env.CLAUDE_CODE_OAUTH_TOKEN = freshToken;
				console.log(
					"[claude-env] Refreshed expired token from credentials file",
				);
				return env;
			}
		} catch {
		...
		console.log(
			"[claude-env] Access token expired, no fresh token in credentials file",
		);
		return env;
	}
	...
	try {
		...
		console.warn(
			"[claude-env] Could not read OAuth credentials:",
			(e as Error).message,
		);
	}
...
export function ensureOnboardingComplete(
	...
	try {
		...
		if (changed) {
			...
			if (written !== content) {
				console.error(
					"[claude-env] Config write verification failed — content mismatch",
				);
			} else {
				console.log(
					"[claude-env] Set hasCompletedOnboarding=true, hasTrustDialogAccepted=true, theme=dark",
				);
			}
		}
	} catch (e) {
		console.error(
			"[claude-env] Failed to update .claude.json:",
			(e as Error).message,
		);
	}
}
	...
	for (const [key, value] of Object.entries(process.env)) {
		...
		cleanEnv[key] =
			value.length > MAX_ENV_VALUE_LENGTH
				? (console.warn(
						`[claude-env] Truncating oversized env var ${key} (${value.length} bytes)`,
					),
					value.slice(0, MAX_ENV_VALUE_LENGTH))
				: value;
	}
	return cleanEnv;
}
