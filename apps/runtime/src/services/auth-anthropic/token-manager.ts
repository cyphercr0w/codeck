...
export function getInMemoryToken(): string | null {
	...
	if (inMemoryTokenExpiresAt > 0 && Date.now() >= inMemoryTokenExpiresAt)
		return null;
...
/** Update in-memory token cache (e.g., after reading a refreshed token from credentials file) */
...
export function setAuthCache(cache: {
	checked: boolean;
	authenticated: boolean;
	checkedAt: number;
}): void {
...
export function getRefreshMonitorInterval(): ReturnType<
	typeof setInterval
> | null {
...
function validateCredentialsPermissions(): boolean {
	try {
		...
		if ((stat.mode & 0o077) !== 0) {
			console.log(
				"[Claude] ⚠ Credentials file has insecure permissions:",
				"0o" + (stat.mode & 0o777).toString(8),
			);
		...
		console.log(
			"[Claude] Error checking credentials permissions:",
			(e as Error).message,
		);
		return false;
	}
...
export function backupCredentials(
	...
	try {
		...
		console.warn(
			"[Claude] Failed to write encrypted credentials backup:",
			(e as Error).message,
		);
	}
}
...
function restoreCredentials(): boolean {
	...
	try {
		...
		if (raw.version === 2 && raw.claudeAiOauth?.accessToken?.encrypted) {
			...
			const refreshToken = raw.claudeAiOauth.refreshToken
				? decryptValue(raw.claudeAiOauth.refreshToken)
				: "";
			const plain = {
				claudeAiOauth: {
					accessToken: token,
			...
			console.log(
				"[Claude] Restored .credentials.json from encrypted backup (plaintext)",
			);
			return true;
		}
		...
		atomicWriteFileSync(
			CLAUDE_CREDENTIALS_PATH,
			readFileSync(CREDENTIALS_BACKUP, "utf-8"),
			{ mode: 0o600 },
		);
		...
		console.error(
			"[Claude] Failed to restore credentials:",
			(e as Error).message,
		);
		return false;
	}
...
export function readCredentials(): PlaintextCredentials | null {
	...
	try {
		...
		console.log(
			"[Claude] Error reading credentials, trying encrypted backup:",
			(e as Error).message,
		);
		return readEncryptedBackup();
	}
...
export function saveOAuthToken(
	...
	atomicWriteFileSync(
		CLAUDE_CREDENTIALS_PATH,
		JSON.stringify(plainCredentials, null, 2),
		{ mode: 0o600 },
	);
	...
	console.log(
		`[Claude] ✓ Token saved (plaintext + encrypted backup, expires in ${expiresInHours}h, refresh_token: ${refreshToken ? "yes" : "no"})`,
	);
	return true;
}
...
export async function performTokenRefresh(
	...
	try {
		...
		if (tokenData.error || !tokenData.access_token) {
			console.log(
				"[Claude] Token refresh error:",
				tokenData.error || "no access_token",
			);
			return false;
		}
...
export function clearAllCredentialFiles(): void {
	...
	try {
		if (existsSync(CREDENTIALS_BACKUP)) unlinkSync(CREDENTIALS_BACKUP);
	} catch {
		/* ignore */
	}
	try {
		if (existsSync(CLAUDE_CREDENTIALS_PATH))
			unlinkSync(CLAUDE_CREDENTIALS_PATH);
	} catch {
		/* ignore */
	}
	try {
		if (existsSync(ACCOUNT_INFO_CACHE_PATH))
			unlinkSync(ACCOUNT_INFO_CACHE_PATH);
	} catch {
		/* ignore */
	}
...
export function markTokenExpired(): void {
	...
	if (
		lastTokenSaveAt > 0 &&
		Date.now() - lastTokenSaveAt < FRESH_TOKEN_GRACE_MS
	) {
		console.log(
			"[Claude] markTokenExpired ignored — token was just saved (<10s ago), stale PTY signal discarded",
		);
		return;
	}
	...
	if (creds?.claudeAiOauth?.refreshToken) {
		console.log(
			"[Claude] Attempting token refresh before clearing credentials...",
		);
...
export function syncCredentialsAfterCLI(): void {
	try {
		...
		if (raw.version === 2 && raw.claudeAiOauth?.accessToken?.encrypted) {
			...
			if (token && isRealToken(token)) {
				...
				const expiresAt: number =
					raw.claudeAiOauth.expiresAt ?? Date.now() + 365 * 24 * 60 * 60 * 1000;
				const expiresInVal = Math.max(
					0,
					Math.round((expiresAt - Date.now()) / 1000),
				);
				const accountInfoVal =
					raw.accountInfo ?? getCachedAccountInfo() ?? undefined;
				saveOAuthToken(
					token,
					refreshToken,
					accountInfoVal || undefined,
					expiresInVal,
				);
				console.log(
					"[Claude] Migrated v2 encrypted credentials to plaintext after CLI execution",
				);
			}
			return;
		}
		...
		if (
			...
			typeof raw.claudeAiOauth.accessToken === "string"
		) {
			const token = raw.claudeAiOauth.accessToken;
			if (!isRealToken(token)) {
				if (token.includes("mock"))
					console.log(
						"[Claude] Ignoring mock token from CLI in .credentials.json",
					);
				return;
			}
			if (token === inMemoryToken) return; // no change
			const refreshToken =
				typeof raw.claudeAiOauth?.refreshToken === "string"
		...
		console.warn(
			"[Claude] syncCredentialsAfterCLI error:",
			(e as Error).message,
		);
	}
}
...
export function startTokenRefreshMonitor(
	...
	console.log(
		"[Claude] Starting token refresh monitor (every 5min, 30min proactive margin)",
	);
	...
	refreshMonitorInterval = setInterval(async () => {
		...
		if (timeUntilExpiry <= 0) {
			...
			if (hasRefreshToken) {
				console.log(
					"[Claude] Token expired, attempting recovery via refresh token...",
				);
				const ok = await performTokenRefresh(creds.claudeAiOauth.refreshToken!);
				if (!ok) {
					console.log(
						"[Claude] Refresh failed — token is dead, user must re-login",
					);
					refreshMonitorBroadcast?.({ type: "auth:expired" });
				}
			} else {
				console.log(
					"[Claude] Token expired with no refresh token available — user must re-login",
				);
				refreshMonitorBroadcast?.({ type: "auth:expired" });
			}
			...
			if (hasRefreshToken) {
				console.log(
					`[Claude] Token expires in ${Math.round(timeUntilExpiry / 60000)}min, refreshing proactively...`,
				);
				const ok = await performTokenRefresh(creds.claudeAiOauth.refreshToken!);
				if (!ok) {
					console.log("[Claude] Proactive refresh failed — notifying frontend");
					refreshMonitorBroadcast?.({
						type: "auth:expiring",
						minutesLeft: Math.round(timeUntilExpiry / 60000),
					});
				}
			} else {
				console.log(
					`[Claude] Token expires in ${Math.round(timeUntilExpiry / 60000)}min, no refresh token — notifying frontend`,
				);
				refreshMonitorBroadcast?.({
					type: "auth:expiring",
					minutesLeft: Math.round(timeUntilExpiry / 60000),
			...
			console.log(
				`[Claude] Token expires in ${Math.round(timeUntilExpiry / 60000)}min`,
			);
		}
...
try {
			...
			if (filename === ".credentials.json") {
				if (!existsSync(CLAUDE_CREDENTIALS_PATH)) {
					...
					credentialsRestoreTimer = setTimeout(() => {
						if (!existsSync(CLAUDE_CREDENTIALS_PATH)) {
							console.log(
								`[Claude] WATCH: .credentials.json confirmed DELETED — ${new Date().toISOString()}`,
							);
							if (existsSync(CREDENTIALS_BACKUP)) {
								try {
									...
									console.log(
										"[Claude] WATCH: auto-restored credentials from backup",
									);
								} catch (e) {
									console.error(
										"[Claude] WATCH: auto-restore failed:",
										(e as Error).message,
									);
								}
							}
					...
					try {
						...
						if (
							...
							if (isRealToken(token) && token !== inMemoryToken) {
								...
								const expiresAt: number = raw.claudeAiOauth?.expiresAt ?? 0;
								console.log(
									"[Claude] WATCH: CLI refreshed token — syncing to in-memory state",
								);
								inMemoryToken = token;
								inMemoryTokenExpiresAt = expiresAt;
								tokenMarkedExpired = false;
	...
	console.warn(
		"[Claude] Could not set up credentials watcher:",
		(e as Error).message,
	);
}
...
try {
	...
	if (
		_creds?.claudeAiOauth?.accessToken &&
		isRealToken(_creds.claudeAiOauth.accessToken)
	) {
	...
	/* non-fatal — first WS connect will do a lazy read */
}
