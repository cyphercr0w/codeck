...
function TokenOptimizationCard() {
	...
	async function loadSettings() {
		try {
			const res = await apiFetch("/api/system/token-settings");
			const data = await res.json();
			setSettings(data);
		} catch {
			/* ignore */
	...
	const effortLevels: Array<"low" | "medium" | "high"> = [
		"low",
		"medium",
		"high",
	];
		...
		<div class="dash-card">
			...
			<div style={rowStyle}>
				<div style={labelColStyle}>
					...
					<div style={descStyle}>
						Lower = more aggressive compaction, saves tokens in long sessions.
						Opus 1M recommended: 30%. Sonnet 200K: 50%.
					</div>
				</div>
			...
			<div style={rowStyle}>
				<div style={labelColStyle}>
					...
					<div style={descStyle}>
						Low = fast & cheap. Medium = balanced. High = thorough reasoning.
					</div>
				</div>
				<div style={controlColStyle}>
					<div style={{ display: "flex" }}>
						{effortLevels.map((level, i) => (
							<button
								key={level}
								onClick={() => update("effortLevel", level)}
								style={{
									padding: "4px 10px",
									fontSize: "12px",
			...
			<div style={rowStyle}>
				<div style={labelColStyle}>
					...
					<div style={descStyle}>
						Defer MCP tool definitions when they exceed X% of context. Lower =
						more tools loaded on-demand.
					</div>
				</div>
				<div style={controlColStyle}>
					<input
			...
			<div style={{ ...rowStyle, borderBottom: "none" }}>
				<div style={labelColStyle}>
					...
					<div style={descStyle}>
						Cap on extended thinking budget per request. Lower = cheaper but
				...
				<div style={controlColStyle}>
					...
					/>
					<span
						style={{ fontSize: "12px", minWidth: "48px", textAlign: "right" }}
					>
		...
		<div class="content-section">
			<div class="home-content">
				...
				<div style="margin-bottom: 24px; padding-top: 24px; border-top: 1px solid var(--border)">
					<h3 class="dash-title" style="margin-bottom: 16px">
						<IconActivity size={14} /> Token Optimization
					</h3>
