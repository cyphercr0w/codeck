...
describe('SSH Routes', () => {
  ...
  beforeEach(() => {
    ...
    mockGenerateSSHKey.mockReturnValue({ success: true, publicKey: 'ssh-ed25519 AAAA...' });
  ...
  describe('GET /status', () => {
    it('should return hasKey: true when SSH key exists', async () => {
      ...
      expect(response.body).toEqual({ hasKey: true });
    ...
    it('should return hasKey: false when no SSH key', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toEqual({ hasKey: false });
    });
  });
  ...
  describe('POST /generate', () => {
    ...
    it('should not force regenerate when force is not explicitly true', async () => {
      mockGenerateSSHKey.mockReturnValue({ success: true, publicKey: 'ssh-ed25519 CCCC...' });
    ...
    it('should broadcast status after generation', async () => {
      mockGenerateSSHKey.mockReturnValue({ success: true, publicKey: 'ssh-ed25519...' });
  ...
  describe('GET /public-key', () => {
    ...
    it('should return 500 when public key is not available', async () => {
      ...
      expect(response.body).toEqual({ success: false, error: 'Could not get public key' });
    });
