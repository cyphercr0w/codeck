 ...
 */
/**
 * @param refocus — if false, skip re-focusing hidden input (e.g. for toggle buttons
 *   where opening the keyboard is unwanted).
 */
function tap(fn: () => void, refocus = true) {
	return {
...
export function MobileTerminalToolbar() {
	const [expanded, setExpanded] = useState(() => {
		try {
			return localStorage.getItem("codeck-mobile-keys") !== "hidden";
		} catch {
			return true;
		}
	});
	...
	const toggleExpanded = useCallback(() => {
		setExpanded((prev) => {
			const next = !prev;
			try {
				localStorage.setItem("codeck-mobile-keys", next ? "visible" : "hidden");
			} catch {}
			return next;
		});
	}, []);
	...
	const lastSent = useRef<{ text: string; time: number }>({
		text: "",
		time: 0,
	});
	...
	const handleInput = useCallback(
		(e: Event) => {
			...
			if (text) {
				...
				if (
					text === lastSent.current.text &&
					now - lastSent.current.time < 50
				) {
					resetInput();
					return;
				}
	...
	return (
		<>
			...
			{/* Fixed toolbar */}
			<div
				class={`mobile-toolbar${expanded ? "" : " collapsed"}`}
				role="toolbar"
			...
			>
				{expanded ? (
					<>
						...
						<div class="mobile-toolbar-row">
							{(
								...
								<button
									key={key}
									class="mobile-nav-key"
									{...tap(() => handleNavKey(key))}
									aria-label={key}
								>
									{symbol}
								</button>
							...
							<button
								class="mobile-nav-key primary"
								{...tap(() => handleNavKey("Enter"))}
								aria-label="Enter"
							>
								↵
							</button>
							<button
								class="mobile-nav-key"
								{...tap(() => handleNavKey("Tab"))}
								aria-label="Tab"
							>
								⇥
							</button>
							<button
								class="mobile-nav-key esc"
								{...tap(() => handleNavKey("Escape"))}
								aria-label="Escape"
							>
								ESC
							</button>
					...
					<button
						class="mobile-collapsed-bar"
						{...tap(toggleExpanded, false)}
						aria-label="Show special keys"
					>
...
function MobileLimitsBar() {
	...
	const uptime = activeSession
		? Math.floor((Date.now() - activeSession.createdAt) / 60000)
		: 0;
	...
	return (
		<div class="mobile-limits-bar">
			<div class="mobile-limits-items">
									...
									<div
										...
										style={
											filled
												...
												: partial
													? {
															background: `linear-gradient(to right, ${barColor(pct)} ${((pct - segStart) / 20) * 100}%, var(--bg-tertiary, #333) ${((pct - segStart) / 20) * 100}%)`,
														}
													: undefined
										}
									/>
				...
				{ctx && ctx.contextPercent > 0 && usage?.available && (
					<span class="mobile-limits-sep">|</span>
				)}
				{usage?.available && usage.fiveHour && (
					<div class="mobile-limits-item">
						...
						{usage.fiveHour.resetsAt && (
							<span class="mobile-limits-reset">
								{formatReset(usage.fiveHour.resetsAt)}
							</span>
						)}
					</div>
				)}
				{usage?.available && usage.fiveHour && usage.sevenDay && (
					<span class="mobile-limits-sep">|</span>
				)}
				{usage?.available && usage.sevenDay && (
					<div class="mobile-limits-item">
						...
						{usage.sevenDay.resetsAt && (
							<span class="mobile-limits-reset">
								{formatReset(usage.sevenDay.resetsAt)}
							</span>
						)}
					</div>
				)}
			...
			<div class="mobile-limits-status">
				{stale && usage && (
					<span class="tsb-stale" title="Usage may be outdated">
						&#9888;
					</span>
				)}
				<span class={`tsb-dot tsb-dot-${status}`} />
				<span class="mobile-limits-status-text">
